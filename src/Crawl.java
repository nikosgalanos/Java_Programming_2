...
public class Crawl {
	...
	/**
	 * the folder where all the pages will be saved.
	 */
	private String path;
	/**
	 * how much links showed up for 2 or more times.
	 */
	 ...
	 */
	protected void crawl() throws IOException, SQLException {
		...
		for (;;) {
			...
			Iterator<String> i = le.getLinks().iterator();
	...
	/**
	 * Method which prints useful details of the process.
	...
	private void printStatus(int loopCount,  URL urlToBeChecked) {
		System.out.println("** Queue Size: " + linksQueue.size() + " | Links Count: "
				+ finalList.size() + " | Checking " + (loopCount) + " link:    "
				+ urlToBeChecked.toString());
	}

	/**
	 * Method which prints the number of the links saves and also the
	 * links which showed up more than one times.
	 */
	private void printEndStatus() {
		System.out.println("END. " + finalList.size() + " pages were saved. "
				+ "Double links: " + doubleLinks);
	}
	...
	private void saveFile(URL url) throws UnsupportedEncodingException, IOException {
		// if url isn't alredy in the map
	...
	private String setPath() {
		...
		for (;;) {
			...
			// if the user chooses a folder, else if the user presses cancel button
	...
	/**
	 * Method which asks for the user (in a graphic way) to enter
	 * the time which is willing to wait.
	...
	private float getWaitingTime() {
		...
		for (;;) {
			try {
				...
				JOptionPane.showMessageDialog(null, "Error, null or not an integer. Please try again.",
	...
	/**
	 * Method which asks for the user (in a graphic way) to enter
	 * the links he want per html page.
	...
	private void addLinkstoQueue() {
		...
		try {
			linksQueue.add(new URL("http://edition.cnn.com/"));
			linksQueue.add(new URL("https://www.bloomberg.com/europe"));
			linksQueue.add(new URL("http://www.businessinsider.com/"));
			linksQueue.add(new URL("http://www.nationalgeographic.com/"));
			linksQueue.add(new URL("http://www.food.com/"));
			linksQueue.add(new URL("http://www.imdb.com/"));
		} catch (MalformedURLException e) {
			JOptionPane.showMessageDialog(null, "Error with the links inserted.",
					null, JOptionPane.ERROR_MESSAGE);
		}
	}
